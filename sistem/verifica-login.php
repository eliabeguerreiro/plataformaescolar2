...
if($btnLogin){
	...
	if((!empty($matricula)) AND (!empty($senha))){
		$result_usuario = "SELECT id, nome, matricula, senha, tipo FROM usuarios WHERE matricula='$matricula' LIMIT 1";//talvez isso vá mudar
		...
		if($resultado_usuario){
			...
			if(password_verify($senha, $row_usuario['senha'])){
				...
				$_SESSION['tipo'] = $row_usuario['tipo'];//isso vai musar
				$nh = '';
				...
				header("Location:../src.php?pg=painel");//isso vai mudar
			}else{
				$_SESSION['msg'] = "Login e senha incorreto!";
				header("Location: ../index.php");//isso vai mudar
			}
		}
	}else{
